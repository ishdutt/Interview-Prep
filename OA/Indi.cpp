...
bool solve(vector<int>push_seq, vector<int>pop_seq, stack<int>our_stack, set<int>visited_indices, int interator_for_push_seq, int interator_for_pop_seq, vector<string>&sequence_of_operation)
{
    ...
    if(our_stack.empty() && flag==1 && visited_indices.size()==push_seq.size())
    {
        ...
        return dp[{key,{interator_for_push_seq,interator_for_pop_seq}}]= 1;
    }
    ...
    {
        return dp[{key,{interator_for_push_seq,interator_for_pop_seq}}]= 0;
    }    

    if(!our_stack.empty()&&dp.find({key,{interator_for_push_seq,interator_for_pop_seq}})!=dp.end())
    {
        cout<<"TABLE SE VALUE LIYA="<<interator_for_push_seq<<" "<<interator_for_pop_seq<<" "<<dp[{key,{interator_for_push_seq,interator_for_pop_seq}}]<<endl;
        return dp[{key,{interator_for_push_seq,interator_for_pop_seq}}];
    }  
